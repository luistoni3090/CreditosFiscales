...
namespace Px_Controles.Controls.DataGridView
{
    ...
    public partial class UIDataGridView : UserControl
    {
        #region 属性
        /// <summary>
        ...
        /// The m row type
        /// </summary>
        private Type m_rowType = typeof(UIDataGridViewRow);
        /// <summary>
        /// 行元素类型，默认UIDataGridViewItem
        /// </summary>
        ...
        [Description("行控件类型，默认UIDataGridViewRow，如果不满足请自定义行控件实现接口IDataGridViewRow"), Category("自定义")]
        public Type RowType
        {
        ...
        private List<IDataGridViewRow> GetSelectRows()
        {
            ...
            if (Rows != null && Rows.Count > 0 && m_rowType == typeof(UIDataGridViewTreeRow))
            {
                foreach (UIDataGridViewTreeRow row in Rows)
                {
        ...
        private List<IDataGridViewRow> FindTreeRowSelected(UIDataGridViewTreeRow row)
        {
            List<IDataGridViewRow> lst = new List<IDataGridViewRow>();
            var _lst = row.ChildrenRows.FindAll(p => p.IsChecked);
            lst.AddRange(_lst);
            foreach (UIDataGridViewTreeRow _row in row.ChildrenRows)
            {
        ...
        /// <returns>UIDataGridView.</returns>
        private UIDataGridView FindChildGrid(Control c)
        {
            foreach (Control item in c.Controls)
            {
                if (item is UIDataGridView)
                    return item as UIDataGridView;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="UIDataGridView" /> class.
        /// </summary>
        ...
        private void UIDataGridView_SizeChanged(object sender, EventArgs e)
        {
            //LoadColumns();
            //ReloadSource();
        }

        private void UIDataGridView_Scroll(object sender, ScrollEventArgs e)
        {
