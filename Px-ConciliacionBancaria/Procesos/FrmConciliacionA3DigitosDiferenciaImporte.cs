...
{
    public partial class FrmConciliacionA3DigitosDiferenciaImporte : FormaGenBar
    {
        ...
        public FrmConciliacionA3DigitosDiferenciaImporte()
        {
            InitializeComponent();
            Start();
        }
        ...
        private async void detConciliar(object sender, EventArgs e)
        {
            ...
            await _Main.Status($"¿Deseas hacer la Conciliación con Diferencia de importe a 3 Dígitos?", (int)MensajeTipo.Question);
            if (MessageBoxMX.ShowDialog(null, $"¿Deseas hacer la Conciliación con Diferencia de importe a 3 Dígitos?", "Precaución", (int)StatusColorsTypes.Question, true) == System.Windows.Forms.DialogResult.Cancel)
                return;
            ...
            var resultado = await UeConciliadaAsync(IdBanco, IdCuenta);
        ...
        public async Task<int> UeConciliadaAsync(string IdBanco, string IdCuenta)
        {
            // Declaración de variables
            string Ls_Status = null;

            // Crear consulta para obtener el estado
            ...
            if (oRes.Err == 0 && oRes.Data.Tables.Count > 0)
            {
                // Mapear el estado de conciliación
            ...
            else if (oRes.Err == -1)
            {
                ...
                MessageBoxMX.ShowDialog(
                    null,
                    $"Error al buscar Número de Conciliación: {oRes.Message}",
                    "Error",
            ...
            // Validar el estado de conciliación
            if (string.IsNullOrEmpty(Ls_Status) || Ls_Status != "K")
            {
                MessageBoxMX.ShowDialog(
                    null,
            ...
            // Si todo está correcto, retornar 1
            return 1;
        }
        ...
        public async Task<int> UeMovtosConciliados(string Ii_Banco, string Ii_Cuenta)
        {
            ...
            if (checkConciliaSimilitud.Checked)
            {
                ...
                if (oRes.Err != 0)
                {
                    // Manejo de errores en el procedimiento
                    MessageBoxMX.ShowDialog(
                        null,
                        oRes.Message,
                        "Error en P_CON_DIF_SREF3",
                        (int)StatusColorsTypes.Danger,
                        false
                    );
                    return 0;
        ...
        {
            // Declaración de variables
            DateTime Ld_Fecha = Idt_Fecha;
            DateTime? Ldt_FechaAux = null, Ldt_FechaBco = null;
            decimal? Ld_Tolerancia = null, Ld_ToleranciaDlls = null;
            decimal Id_Tolerancia;
            int Li_Res;

            // Crear consulta para obtener los valores

            oReq.Tipo = 0;
                oReq.Query = $@"
                SELECT 
                    ADD_MONTHS(TO_DATE('{Idt_Fecha:yyyy-MM-dd}', 'yyyy-MM-dd'), -VIGENCIAAUX) AS FechaAux,
                    ADD_MONTHS(TO_DATE('{Idt_Fecha:yyyy-MM-dd}', 'yyyy-MM-dd'), -VIGENCIABCO) AS FechaBco,
                    TOLERANCIA, 
                    TOLERANCIADLLS
            ...
            if (oRes.Err == 0 && table.Rows.Count > 0)
            {

                Ldt_FechaAux = Convert.ToDateTime(table.Rows[0]["ADD_MONTHS(TO_DATE('{Idt_Fecha:yyyy-MM-dd}', 'yyyy-MM-dd'), -VIGENCIAAUX)"]);
                Ldt_FechaAux = Convert.ToDateTime(table.Rows[0]["ADD_MONTHS(TO_DATE('{Idt_Fecha:yyyy-MM-dd}', 'yyyy-MM-dd'), -VIGENCIABCO)"]);
                Ld_Tolerancia = Convert.ToDecimal(table.Rows[0]["TOLERANCIA"]);
                Ld_ToleranciaDlls = Convert.ToDecimal(table.Rows[0]["TOLERANCIADLLS"]);
            }
            else if (oRes.Err == -1)
            {
                // Error en la consulta SQL
                MessageBoxMX.ShowDialog(
                    null,
                    $"Error al querer obtener la vigencia",
                    "Error",
                    (int)StatusColorsTypes.Danger,
                    false
                );
            ...
            // Determinar tolerancia según la moneda
            Id_Tolerancia = (Is_Moneda == "P") ? (Ld_Tolerancia ?? 0) : (Ld_ToleranciaDlls ?? 0);

            // Manejo de códigos SQL
            if (oRes.Err == 100)
            {
                // Sin registros
                var Id_VigenciaAux = FProFechaIniMes(Ld_Fecha);
                var Id_VigenciaBco = FProFechaIniMes(Ld_Fecha);
            ...
            // Validar si las fechas son nulas y calcular valores predeterminados
            if (Ldt_FechaAux == null)
                Ldt_FechaAux = FProFechaIniMes(Ld_Fecha);

            if (Ldt_FechaBco == null)
                Ldt_FechaBco = FProFechaIniMes(Ld_Fecha);

            // Actualizar valores de vigencia
            var finalVigenciaAux = FProFechaIniMes((DateTime)Ldt_FechaAux);
            var finalVigenciaBco = FProFechaIniMes((DateTime)Ldt_FechaBco);
