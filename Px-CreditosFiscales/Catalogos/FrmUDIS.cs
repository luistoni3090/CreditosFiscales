...
{
    public partial class FrmUDIS : Form
    {
        public xMain _Main { get; set; }

        eRequest oReq = new eRequest();
        DataGridView oGrid = new DataGridView();
        List<eUDIS> _Con = new List<eUDIS>();
        List<eUDIS> _ConFiltra = new List<eUDIS>();
        eUDIS _Reg3 = new eUDIS();
        ...
        private async Task Inicio()
        {
            ...
            lblTitulo.Text = "Catálogo de UDIS";
        ...
        private async Task Grid1()
        {
            ...
            {
                new DataGridViewTextBoxColumn() { HeaderText = "Fecha ", Width = 200,  DataPropertyName = "FECHA" },
                new DataGridViewTextBoxColumn() { HeaderText = "Unidad", Width = 300,  DataPropertyName = "UNIDAD"  }
            });
        ...
        private async Task conActualiza()
        {
            ...
            TimeSpan ts = DateTime.Now - startTime;
            if (_Main != null)
                await _Main.Status($"Catálogo de UDIS . Tiempo: {ts}", (int)MensajeTipo.Success);
        ...
        private async Task conActualizaDBGrid1()
        {
            ...
            try
            {
                oReq.Query = @"SELECT
                                ...
                                UNIDAD,
                                 (
                                    CASE WHEN length(FECHA) > 0 THEN CAST(FECHA AS VARCHAR(10)) ELSE '' END ||
                                    CASE WHEN length(UNIDAD) > 0 THEN CAST(UNIDAD AS VARCHAR(10)) ELSE '' END
                                ) AS ALLCOLUMNS
                                FROM UDIS";
                var oRes = await WSServicio.Servicio(oReq);

                if (!Object.ReferenceEquals(null, oRes.Data.Tables[0]))
                    _Con = oRes.Data.Tables[0].AListaDe<eUDIS>();
        ...
        private async Task<bool> conValidaSiExisteRegistro(string ANIO, string MES)
        {
            ...
            try
            {
                ...
                if (oRes.Data.Tables.Count > 0 && oRes.Data.Tables[0].Rows.Count > 0){ 
                    if (!Object.ReferenceEquals(null, oRes.Data.Tables[0]))
                        _Reg3 = oRes.Data.Tables[0].Rows[0].RowAObjetoDe<eUDIS>();
                    existe = true;
                }
