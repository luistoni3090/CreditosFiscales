...
namespace Px_CreditosFiscales.Catalogos
{
    public partial class FrmSecuencia : Form
    {
        ...
        List<eCREDITO_CONCEPTO> _Con = new List<eCREDITO_CONCEPTO>();
        List<eSECUENCIA_CREDITO> _Con2 = new List<eSECUENCIA_CREDITO>();
        List<eSECUENCIA_CREDITO> _ConFiltra = new List<eSECUENCIA_CREDITO>();
        eSECUENCIA_CREDITO _Reg3 = new eSECUENCIA_CREDITO();
        ...
        private async Task Inicio()
        {
            ...
            lblTitulo.Text = "Cátalogo de Secuencia del Crédito";
        ...
        private async Task InicioForma()
        {
            ...
            await GridSECUENCIA_CREDITO();
            await GridCREDITO_CONCEPTO();
        ...
        #region Grid
        private async Task GridSECUENCIA_CREDITO()
        {
            ...
            {
                new DataGridViewTextBoxColumn() { HeaderText = "Sigla", Width = 200,  DataPropertyName = "SIGLA_CPTO" },
        ...
        #region GridSECUENCIA_CREDITO
        private async Task GridCREDITO_CONCEPTO()
        {
            ...
            {
                new DataGridViewTextBoxColumn() { HeaderText = "Municipio", Width = 200,  DataPropertyName = "MUNICIPIO"  },
                new DataGridViewTextBoxColumn() { HeaderText = "Dependencia", Width = 200, DataPropertyName = "DEPENDENCIA"  },
                new DataGridViewTextBoxColumn() { HeaderText = "Año", Width = 100,  DataPropertyName = "ANIO" },
                new DataGridViewTextBoxColumn() { HeaderText = "Secuencia", Width = 100,  DataPropertyName = "SECUENCIA" }
        ...
        private async Task conActualiza()
        {
            ...
            await conActualizaDBGridCREDITO_CONCEPTO();
        ...
        private async Task conActualizaDBGridCREDITO_CONCEPTO()
        {

            try
            {
                oReq.Query = @"SELECT SC.SIGLA_CPTO, DESCR
                                FROM SECUENCIA_CREDITO SC
                                JOIN CREDITO_CONCEPTO CC ON SC.SIGLA_CPTO = CC.SIGLA_CPTO
                                ";
                var oRes = await WSServicio.Servicio(oReq);

                if (!Object.ReferenceEquals(null, oRes.Data.Tables[0]))
                    _Con = oRes.Data.Tables[0].AListaDe<eCREDITO_CONCEPTO>();
        ...
        private async Task conActualiza2(string CLAVE)
        {
            ...
            await conActualizaDBGridSECUENCIA_CREDITO(CLAVE);
        ...
        private async Task conActualizaDBGridSECUENCIA_CREDITO(string SIGLA)
        {
            ...
            {
                oReq.Query = @"SELECT
                            MU.DESCR AS MUNICIPIO,
                            CASE 
                                WHEN SIGLA_DEPEN = 'P' THEN 'PESOS'
                                WHEN SIGLA_DEPEN = 'R' THEN 'RECAUDACION DE RENTAS'
                                WHEN SIGLA_DEPEN = 'A' THEN 'AUDITORIA FISCAL'
                                WHEN SIGLA_DEPEN = 'I' THEN 'DIRECCION DE INGRESOS'
                                ELSE ''
                            END AS DEPENDENCIA,
                            ANIO,
                            SECUENCIA,
                             (
                                CASE WHEN length(MUNICIPIO) > 0 THEN CAST(MUNICIPIO AS VARCHAR(10)) ELSE '' END ||
                                CASE WHEN length(SIGLA_DEPEN) > 0 THEN CAST(SIGLA_DEPEN AS VARCHAR(10)) ELSE '' END ||
                                CASE WHEN length(ANIO) > 0 THEN CAST(ANIO AS VARCHAR(10)) ELSE '' END ||
                                CASE WHEN length(SECUENCIA) > 0 THEN CAST(SECUENCIA AS VARCHAR(10)) ELSE '' END
                            ) AS ALLCOLUMNS
                            FROM SECUENCIA_CREDITO SC
                            JOIN MUNICIPIO MU ON SIGLA_MPO = ID_MUNICIPIO
                            WHERE SIGLA_CPTO = '"+SIGLA+ "' ORDER BY ANIO ASC ";

                var oRes = await WSServicio.Servicio(oReq);

                if (!Object.ReferenceEquals(null, oRes.Data.Tables[0]))
                    _Con2 = oRes.Data.Tables[0].AListaDe<eSECUENCIA_CREDITO>();
        ...
        private async void oGrid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            string SIGLA = oGrid.Rows[e.RowIndex].Cells[0].Value.ToString();
            await conActualiza2(SIGLA);
        ...
        private async Task<bool> conValidaSiExisteRegistro(string Inferior, string Superior, string Clave, string Tipo)
        {
            ...
            try
            {
                ...
                if (oRes.Data.Tables.Count > 0 && oRes.Data.Tables[0].Rows.Count > 0){ 
                    if (!Object.ReferenceEquals(null, oRes.Data.Tables[0]))
                        _Reg3 = oRes.Data.Tables[0].Rows[0].RowAObjetoDe<eSECUENCIA_CREDITO>();
                    existe = true;
                }
