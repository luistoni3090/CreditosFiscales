...
{
    public partial class FrmStatusCredito : Form
    {
        public xMain _Main { get; set; }

        eRequest oReq = new eRequest();
        DataGridView oGrid = new DataGridView();
        List<eSTATUS_CREDITO> _Con = new List<eSTATUS_CREDITO>();
        List<eSTATUS_CREDITO> _ConFiltra = new List<eSTATUS_CREDITO>();
        eSTATUS_CREDITO _Reg3 = new eSTATUS_CREDITO();

        #region Contructor
        public FrmStatusCredito()
        {
            InitializeComponent();
        ...
        private async Task Inicio()
        {
            ...
            lblTitulo.Text = "Catálogo del Status del Crédito";
        ...
        private async Task conActualiza()
        {
            ...
            TimeSpan ts = DateTime.Now - startTime;
            if (_Main != null)
                await _Main.Status($"Catálogo del Status del Crédito . Tiempo: {ts}", (int)MensajeTipo.Success);
        ...
        private async Task conActualizaDBGrid1()
        {
            ...
            try
            {
                ...
                if (!Object.ReferenceEquals(null, oRes.Data.Tables[0]))
                    _Con = oRes.Data.Tables[0].AListaDe<eSTATUS_CREDITO>();
        ...
        private async Task<bool> conValidaSiExisteRegistro(string ANIO, string MES)
        {
            ...
            try
            {
                ...
                if (oRes.Data.Tables.Count > 0 && oRes.Data.Tables[0].Rows.Count > 0){ 
                    if (!Object.ReferenceEquals(null, oRes.Data.Tables[0]))
                        _Reg3 = oRes.Data.Tables[0].Rows[0].RowAObjetoDe<eSTATUS_CREDITO>();
                    existe = true;
                }
