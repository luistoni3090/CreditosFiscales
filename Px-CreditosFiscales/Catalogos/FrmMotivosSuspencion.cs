...
{
    public partial class FrmMotivosSuspencion : Form
    {
        public xMain _Main { get; set; }

        eRequest oReq = new eRequest();
        DataGridView oGrid = new DataGridView();
        List<eMOTIVOSSUSPENCION> _Con = new List<eMOTIVOSSUSPENCION>();
        List<eMOTIVOSSUSPENCION> _ConFiltra = new List<eMOTIVOSSUSPENCION>();
        eMOTIVOSSUSPENCION _Reg3 = new eMOTIVOSSUSPENCION();

        #region Contructor
        public FrmMotivosSuspencion()
        {
            InitializeComponent();
        ...
        private async Task Inicio()
        {
            ...
            lblTitulo.Text = "Catálogo de Motivos Suspención";
        ...
        private async Task Grid1()
        {
            ...
            oGrid.Columns.AddRange(new DataGridViewColumn[]
            {
                ...
                new DataGridViewTextBoxColumn() { HeaderText = "Cuenta Contable", Width = 100,  DataPropertyName = "CUENTA_CONTABLE"  },
                new DataGridViewTextBoxColumn() { HeaderText = "Genera Momento", Width = 100,  DataPropertyName = "GENERA_MOMENTO"  },
                new DataGridViewTextBoxColumn() { HeaderText = "Plazo Máximo", Width = 100,  DataPropertyName = "PLAZO_MAX"  }
            });
        ...
        private async Task conActualiza()
        {
            ...
            TimeSpan ts = DateTime.Now - startTime;
            if (_Main != null)
                await _Main.Status($"Catálogo de Motivos de Suspención. Tiempo: {ts}", (int)MensajeTipo.Success);
        ...
        private async Task conActualizaDBGrid1()
        {
            ...
            try
            {
                oReq.Query = @"SELECT MOTIVO, DESCR,
                                CTAPTE_N1 || ',' || CTAPTE_N2 || ',' || CTAPTE_N3 || ',' || CTAPTE_N4 || ',' || CTAPTE_N5 || ',' || CTAPTE_N6 AS CUENTA_CONTABLE,
                                CASE 
                                ...
                                PLAZO_MAX,
                                 (
                                    ...
                                    CASE WHEN length(PLAZO_MAX) > 0 THEN CAST(PLAZO_MAX AS VARCHAR(10)) ELSE '' END
                                ) AS ALLCOLUMNS
                                FROM MOTIVO_SUSP_EJEC";
                  var oRes = await WSServicio.Servicio(oReq);

                if (!Object.ReferenceEquals(null, oRes.Data.Tables[0]))
                    _Con = oRes.Data.Tables[0].AListaDe<eMOTIVOSSUSPENCION>();
        ...
        private async Task<bool> conValidaSiExisteRegistro(string ANIO, string MES)
        {
            ...
            try
            {
                ...
                if (oRes.Data.Tables.Count > 0 && oRes.Data.Tables[0].Rows.Count > 0){ 
                    if (!Object.ReferenceEquals(null, oRes.Data.Tables[0]))
                        _Reg3 = oRes.Data.Tables[0].Rows[0].RowAObjetoDe<eMOTIVOSSUSPENCION>();
                    existe = true;
                }
