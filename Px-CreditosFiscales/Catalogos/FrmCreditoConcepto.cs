...
{
    public partial class FrmCreditoConcepto : Form
    {
        public xMain _Main { get; set; }

        eRequest oReq = new eRequest();
        DataGridView oGrid = new DataGridView();
        List<eCREDITOCONCEPTO> _Con = new List<eCREDITOCONCEPTO>();
        List<eCREDITOCONCEPTO> _ConFiltra = new List<eCREDITOCONCEPTO>();
        eCREDITOCONCEPTO _Reg3 = new eCREDITOCONCEPTO();
        ...
        private async Task Inicio()
        {
            ...
            lblTitulo.Text = "Catálogo de Crédito Concepto";
        ...
        private async Task Grid1()
        {
            ...
            {
                new DataGridViewTextBoxColumn() { HeaderText = "Sigla Concepto ", Width = 100,  DataPropertyName = "SIGLA_CPTO" },
        ...
        private async Task conActualiza()
        {
            ...
            TimeSpan ts = DateTime.Now - startTime;
            if (_Main != null)
                await _Main.Status($"Catálogo de Crédito Concepto. Tiempo: {ts}", (int)MensajeTipo.Success);
        ...
        private async Task conActualizaDBGrid1()
        {
            ...
            try
            {
                oReq.Query = @"SELECT SIGLA_CPTO , DESCR, STATUS,
                                 (
                                    CASE WHEN length(SIGLA_CPTO) > 0 THEN CAST(SIGLA_CPTO AS VARCHAR(10)) ELSE '' END ||
                                    CASE WHEN length(DESCR) > 0 THEN CAST(DESCR AS VARCHAR(10)) ELSE '' END ||
                                    CASE WHEN length(STATUS) > 0 THEN CAST(STATUS AS VARCHAR(10)) ELSE '' END
                                ) AS ALLCOLUMNS
                                FROM CREDITO_CONCEPTO";
                var oRes = await WSServicio.Servicio(oReq);

                if (!Object.ReferenceEquals(null, oRes.Data.Tables[0]))
                    _Con = oRes.Data.Tables[0].AListaDe<eCREDITOCONCEPTO>();
        ...
        private async Task<bool> conValidaSiExisteRegistro(string ANIO, string MES)
        {
            ...
            try
            {
                ...
                if (oRes.Data.Tables.Count > 0 && oRes.Data.Tables[0].Rows.Count > 0){ 
                    if (!Object.ReferenceEquals(null, oRes.Data.Tables[0]))
                        _Reg3 = oRes.Data.Tables[0].Rows[0].RowAObjetoDe<eCREDITOCONCEPTO>();
                    existe = true;
                }
